...
export class CompanionAdapterServer {
    ...
    private async setup() {
        this.inputManager.registerXKeysJoystickCallback(
            (value) => this.onJoyStickSignal(value)
        );
    }


    private async onJoyStickSignal(value: XKeysJoystickValue) {
    ...
    private async cancelMovement() {
        ...
        await this.runRequest(this.lastDirection, Movement.STOP);
    }
    ...
    private async sendDirection() {
        ...
        await this.runRequest(this.direction, Movement.START);
    }
    ...
    private async runRequest(direction: JoystickDirection, movement: Movement) {
        ...
        const { error } = await asyncCall(axios(request));
